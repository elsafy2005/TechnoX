...
const RATINGS_KEY = 'technox_ratings';
const REVIEWS_KEY = 'technox_reviews';

function getAllRatings(){try{return JSON.parse(localStorage.getItem(RATINGS_KEY))||{};}catch{return {};}}
function getRating(pid){const all=getAllRatings();const r=all[pid];if(!r||r.count===0)return{avg:0,count:0};return{avg:r.total/r.count,count:r.count};}
function saveRating(pid,rating){const all=getAllRatings();if(!all[pid])all[pid]={total:0,count:0};all[pid].total+=rating;all[pid].count+=1;localStorage.setItem(RATINGS_KEY,JSON.stringify(all));}
function getAllReviews(){try{return JSON.parse(localStorage.getItem(REVIEWS_KEY))||[];}catch{return [];}}
function saveReview(review){const reviews=getAllReviews();reviews.unshift(review);localStorage.setItem(REVIEWS_KEY,JSON.stringify(reviews));}

function renderStars(val,size){
  size=size||14;let html='';
  for(let i=1;i<=5;i++)html+=`<span style="color:${i<=val?'#f5a623':'#444'};font-size:${size}px;line-height:1;">★</span>`;
  return html;
}
